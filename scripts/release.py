...
_CHANGELOG_TRAILER_RE = re.compile(
    r"^changelog\s*:\s*(.+?)\s*$", re.IGNORECASE | re.MULTILINE
)
...
def _parse_commit(message: str) -> tuple[str, str]:
    ...
    if not m:
        ...
        commit_type, breaking, description = (
            m.group(1).lower(),
            m.group(2),
            m.group(3).strip(),
        )
        section = (
            "🔄 Changed"
            if breaking
            else _COMMIT_TYPE_TO_SECTION.get(commit_type, "🔄 Changed")
        )
        if breaking:
...
def cmd_bump(args) -> None:
    ...
    if args.bump == "finalize":
        ...
        if args.prerelease:
            print(
                "ERROR: --prerelease cannot be combined with bump=finalize",
                file=sys.stderr,
            )
            sys.exit(1)
...
def main() -> None:
    ...
    bump_p.add_argument(
        "bump", choices=["major", "minor", "patch", "prerelease", "finalize"]
    )
    bump_p.add_argument(
        "--prerelease",
